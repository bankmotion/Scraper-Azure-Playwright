...
export const downloadBrowserFromAzure = async (): Promise<void> => {
  const blobSerivceClient = BlobServiceClient.fromConnectionString(config.azureStoConnectionStr)
  const containerClient = blobSerivceClient.getContainerClient(config.blobContainerName)
...
const uploadBrowserToAzure = async (filePath: string): Promise<void> => {
  const blobServiceClient = BlobServiceClient.fromConnectionString(config.azureStoConnectionStr)
